...
void Random::Initialize(DirectXCommon* dxCommon) {
	dxCommon_ = dxCommon;
...
void Random::GraphicsPipeline() {
	...
	Microsoft::WRL::ComPtr<IDxcBlob> pixelShaderBlob = dxCommon_->CompileShader(L"resource/shaders/Random.PS.hlsl", L"ps_6_0");
	...
	RandomResoure = dxCommon_->CreateBufferResource(sizeof(RandomFunction));
	RandomResoure->Map(0, nullptr, reinterpret_cast<void**>(&randomFunction));

	randomFunction->randomeTimer = 0.0f;
}
...
void Random::Command() {
	...
	dxCommon_->GetCommandList()->SetGraphicsRootConstantBufferView(1, RandomResoure->GetGPUVirtualAddress());
