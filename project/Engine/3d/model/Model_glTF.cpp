...
namespace EngineLayer {
	...
	void Model_glTF::InitVertexResource(ModelData modelData) {
		...
		vertexR = D3D12ResourceManager::GetInstance().CreateBufferResource(sizeof(VertexData) * modelData.vertices.size());
	...
	void Model_glTF::InitIndexResource(ModelData modelData) {
		...
		indexR = D3D12ResourceManager::GetInstance().CreateBufferResource(sizeof(uint32_t) * modelData.indices.size());
	...
	SkinCluster Model_glTF::CreateSkinCluster(const Skeleton& skeleton, const ModelDataMulti& modelData) {
		...
		///t0
		// paletteResource確保
		skinCluster.paletteResource = D3D12ResourceManager::GetInstance().CreateBufferResource(sizeof(WellForGPU) * skeleton.joints.size());
		...
		skinCluster.influenceResource = D3D12ResourceManager::GetInstance().CreateBufferResource(sizeof(VertexInfluence) * all_vertex);
