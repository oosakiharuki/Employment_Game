...
void MoveCommand::Execute(PlayerCommand* player) {
	//動く系の処理(スピードの調整)
...
void JumpCommand::Execute(PlayerCommand* player) {
	//移動も同時に行えるように
...
void FireCommand::Execute(PlayerCommand* player) {
	//移動も同時に行えるように
	moveCommand_ = std::make_unique<MoveCommand>();
	moveCommand_->Execute(player);
	//発泡処理
...
void ShieldCommand::Execute(PlayerCommand* player) {
	//移動も同時に行えるように
	moveCommand_ = std::make_unique<MoveCommand>();
	moveCommand_->Execute(player);
	//シールド処理
...
void BrinkCommand::Execute(PlayerCommand* player) {
	//ブリンク処理
	player->CommandBrink();
}
std::unique_ptr<Command> PlayerActionCommand::GetCommand() {
	//ブリンク [Jキー、Bボタン + ブリンクの発動条件、発動中はタイマーが切れるまで]
	if (((Input::GetInstance().TriggerKey(DIK_J) || Input::GetInstance().TriggerButton(XINPUT_GAMEPAD_B)) && playerCommand_->BrinkFlag()) || playerCommand_->BrinkTimeMax()) {
